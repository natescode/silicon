...
| 3b | **Incremental parsing** | 🔄 | **M1 implemented 2026-06-02.** `SyntaxTree.withText()` / `withChanges()` now reuse unchanged top-level elements and reparse only the damaged window instead of full-reparsing (exploits Zig-like element independence — see `src/caas/incremental.ts`). Roslyn-style green/red framing; absolute positions kept (M1 reuses only elements whose spans are provably unchanged). Internal-only — public API unchanged; result is byte-identical to a full reparse, guaranteed by a full-reparse fallback + a `SIGIL_INCREMENTAL_VERIFY=1` tripwire + an equivalence/fuzz harness. ~1.4–2.1× faster with 95–99% element reuse (`bun run bench:incremental`); ceiling is ~2× because lexing is still O(n). **Roadmap:** M2 reuse shifted suffixes via line fix-up; M3 per-node widths + red-tree position authority + typechecker position-service migration (stop storing `sourceLocation`); M4 sub-element sharing + incremental lexing. |
