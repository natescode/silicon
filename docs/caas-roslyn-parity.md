...
| 2c | **`Symbol.containingSymbol`** | ✅ | Implemented 2026-06-01 (forward-compatible shape); **POPULATED 2026-06-12 (S1)**: the lexical binder (`src/ast/binder.ts`, run in `assembleSemanticModel`) surfaces parameters / locals / `@match` pattern fields as Symbols whose `containingSymbol` is the enclosing top-level definition's Symbol (param types read off the containing fn's signature). Top-level definitions stay `undefined`. `findReferences`/`rename` are binding-aware (scope-correct). |
