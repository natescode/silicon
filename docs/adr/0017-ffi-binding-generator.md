...
- **Status:** Accepted — implemented (2026-06-09). The Web `Math`/clock surface is generated from `compiler/bindgen/` and spliced into all three sites between markers; `bun bindgen/cli.ts --check` + `bindgen.test.ts` enforce lockstep. **All source adapters are now SPEC-DRIVEN** (`bun bindgen/cli.ts --report`):
  ...
  The signature + tier come from the spec; only the host accessor (`performance`, `require('node:path')`, `Bun`) is the per-source convention no spec carries. Web IDL gives int-vs-float exactly; Node/Bun use the configurable TS-`number` heuristic (default `Float`). Remaining (additive, not adapter work): wiring the generated Node/Bun/extra-Web bindings into shipped `.si` modules + host shims, Tier-1 (`JSString`) emission, `@webref/idl` per-spec version pinning.
- **Date:** 2026-06-05
- **Deciders:** NatesCode
