...
export function emitModuleSi(ir: BindingIR, provenance: string, strings: StringTier = 'linear'): string {
    ...
    const tierNote = strings === 'jsstring' ? '  (Tier-1: JSString = externref, web/bun only)' : ''
    return [
        '# SPDX-License-Identifier: MIT',
...
export function emitHostModule(ir: BindingIR, indent = '            ', strings: StringTier = 'linear'): string {
    ...
    return ir.decls.map(d => {
        ...
        // Linear String params are i32 ptrs (number); JSString params are JS
        // strings crossing as externref (typed `any` at the JS boundary).
        const ptype = (t: string): string => (js && t === 'String') ? 'any' : 'number'
