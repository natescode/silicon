...
// Two top-level definitions: @fn add (with params) + @let result (with call).
const SRC = '@fn add x, y := { x + y };\n@let result := &add 1, 2;'

// Single let with a literal binding.
const SRC_SIMPLE = '@let x := 42;'
...
describe('SyntaxTree.root', () => {
    ...
    test('withText() returns a new tree with its own root', () => {
        const { tree } = parse(SRC_SIMPLE)
        const { tree: tree2 } = tree.withText('@let m := 99;')
...
describe('SyntaxNode.descendantsOfKind()', () => {
    test('finds all Definition nodes at top level', () => {
        ...
        expect(defs.length).toBeGreaterThanOrEqual(2)  // @fn add + @let result
    })

    test('finds all IntLiteral nodes', () => {
        const { tree } = parse('@let a := 1; @let b := 2;')
