...
const EDITS: Array<{ label: string; steps: string[] }> = [
    ...
    { label: 'append an element',       steps: [BASE + ' s := add(3, 4);'] },
    { label: 'rename a binding',        steps: [BASE.replace('r := add', 'rr := add')] },
    ...
    { label: 'shrink to one element',   steps: ['only := 1;'] },
    ...
    { label: 'inject garbage between elements',
      steps: [BASE.replace('r := add(1, 2)', '@@@ junk\nr := add(1, 2)'), BASE] },
]
...
describe('incremental elaboration actually reuses (E1b)', () => {
    test('editing the last element reuses the first element\'s elaboration by reference', () => {
        ...
        // Edit only the LAST element (the `r` binding literal).
