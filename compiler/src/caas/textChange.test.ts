...
describe('SyntaxTree.withChanges', () => {
    test('returns a ParseResult', () => {
        ...
        const result = tree.withChanges([change(1, 6, 1, 8, '99')])
    ...
    test('the new tree reflects the applied changes', () => {
        const { tree } = parse('x := 42;')
        // Replace '42' with '99'
        const { tree: newTree } = tree.withChanges([change(1, 6, 1, 8, '99')])
        expect(newTree.source).toBe('x := 99;')
    })

    test('preserves the file name from the original tree', () => {
        const { tree } = parse('x := 42;', { file: 'src/main.si' })
        const { tree: newTree } = tree.withChanges([change(1, 6, 1, 8, '99')])
    ...
    test('original tree is not mutated', () => {
        const { tree } = parse('x := 42;')
        tree.withChanges([change(1, 6, 1, 8, '99')])
        expect(tree.source).toBe('x := 42;')
    })
    ...
    test('multi-line change produces a valid parse result', () => {
        ...
        expect(newTree.source).toContain('s := answer();')
    })
