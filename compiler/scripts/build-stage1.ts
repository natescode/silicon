...
// Escape a Silicon source buffer for embedding as a single-quoted
// string literal.  Mirrors the escape set the bootstrap's
// literal_decoded_len / emit_data_segment recognise:
// `\\`, `\'`, `\n`, `\t`, `\r`, `\0`.  Everything else passes through
// as a raw byte (Silicon source is UTF-8).
//
// Returns a Buffer (not a string).  Previously this returned a string
// built via `String.fromCharCode(b)`, which interprets each non-ASCII
// byte as a Latin-1 codepoint that fs.writeFile then re-encodes as a
// 2-byte UTF-8 sequence — mojibake.  Concrete victim: the `→` in
// `src/strata/cast.si:9` was emitted as 6 bytes (0xc3 0xa2 0xc2 0x86
// 0xc2 0x92) instead of the correct 3 (0xe2 0x86 0x92).  Returning a
// Buffer and concatenating at the byte level keeps non-ASCII UTF-8
// sequences intact.
function escapeForSiliconString(buf: Buffer): Buffer {
    // Worst-case output: every byte becomes 2 escape chars.
    const out = Buffer.alloc(buf.length * 2)
    let o = 0
    for (let i = 0; i < buf.length; i++) {
        const b = buf[i]
        if      (b === 0x5c) { out[o++] = 0x5c; out[o++] = 0x5c }
        else if (b === 0x27) { out[o++] = 0x5c; out[o++] = 0x27 }
        else if (b === 0x0a) { out[o++] = 0x5c; out[o++] = 0x6e }  // \n
        else if (b === 0x0d) { out[o++] = 0x5c; out[o++] = 0x72 }  // \r
        else if (b === 0x09) { out[o++] = 0x5c; out[o++] = 0x74 }  // \t
        else if (b === 0x00) { out[o++] = 0x5c; out[o++] = 0x30 }  // \0
        else                 { out[o++] = b }
    }
...
async function generateEmbeddedBundle(): Promise<string> {
    ...
    // Concat at the byte level so the escaped buffer's bytes pass
    // through unchanged (any string round-trip here would re-introduce
    // the Latin-1 mojibake bug that escapeForSiliconString just fixed).
    const file = Buffer.concat([header, escaped, footer])
