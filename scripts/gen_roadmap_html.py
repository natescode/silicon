...
S = [
    ...
    dict(id="F1b", track="ffi", ver="v1.0", size="L", risk="High", crit=True, status="partial",
         note="CORE done (2026-06-09) — blocking <code>@await</code> via Asyncify proven end-to-end under Bun. Shipped: the <code>@await(expr)</code> surface stratum (<code>control.si</code>, transparent + typed to its argument); the Binaryen Asyncify transform (<code>codegen/asyncify.ts</code> <code>applyAsyncify</code>, route-A whole-program); and the host reactor (<code>codegen/async-reactor.ts</code> <code>createAsyncReactor</code> — the unwind→await→rewind driver, single-in-flight, asyncify-stack in a reserved page). <code>asyncify.test.ts</code>: a Silicon <code>@fn</code> <code>@await</code>s host-async imports and resumes correctly — single suspend AND a two-suspend chain (the multi-suspend rewind-state bug found + fixed). REMAINING: the <code>@async</code>/<code>@suspending</code> markers + async effect/coloring rule (P5); precise-coloring route B (recover the size tax); the production <code>sgl run</code> reactor in <code>js-host.ts</code> (auto-apply Asyncify + wire <code>fetch</code>/timers); the JSPI fast path (Phase 2). ADR 0018 Accepted.",
    ...
    dict(id="C2", track="ffi", ver="v1.0", size="L", risk="High", crit=True, gate=True, status="done",
         note="DONE (2026-06-09) — escape gate + host-callable closures end-to-end. (1) The conservative escape/host-reachability classifier + mode-gate (<code>classifyEscapes</code>): a BARE <code>@closure</code> crossing <code>@extern</code> is rejected, pointing at <code>@export_callback</code>. (2) <b>Host-callable closures</b>: <code>@export_callback(closure)</code> (the sanctioned, gate-exempt escape) hands a closure's handle across <code>@extern</code> to a JS/Bun host, which stores it and calls it back later via a synthesized exported <code>__closure_invoke_&lt;k&gt;</code> trampoline with the captured env intact — <b>verified round-trip under Bun</b> (<code>register(7)</code> → host holds handle → <code>__closure_invoke_1(handle, 5)</code> = 35). 7 C2 tests green (gate ×5 + host-callable ×2). <b>Representation</b>: the linear-memory baseline — the handle crosses as i32 and its env is retained in the bump heap (a bounded retention cost). REFINEMENT remaining (§2.2): the leak-free wasm-gc <code>(struct $Clo)</code> + <code>externref</code> form (engine GC; needs new <code>ref.cast</code>/<code>extern.convert</code> ops) that replaces the bump-heap retention. ADR 0019 Accepted.",
