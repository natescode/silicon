...
S = [
    ...
    dict(id="F3", track="ffi", ver="v1.0", size="L", risk="Med", crit=True, status="partial",
         note="CORE done (2026-06-09) — the poll-reactor proven end-to-end. <code>stdlib/future.si</code>: a FUTURE is a no-arg C1 closure over a mutable poll-state pointer that returns a PENDING sentinel until ready; <code>block_on</code> drives one to completion, and <code>block_all</code> drives MANY <b>concurrently</b> — each round polls every still-pending future, so independent futures progress interleaved (the true-concurrency model single-in-flight Asyncify can't express). <code>future.test.ts</code>: block_on (single), block_all (3 futures, deadlines 2/4/3 → 60, polled interleaved), and fast+slow independent progress (→107). Reuses the C1/C2 closure keystone as wake continuations — the reason this waited on closures. <b>UPDATE (2026-06-10): the practical <code>Promise.all</code>-shaped composition this item promises now ALSO ships via the post-gate <code>promise</code> module</b> — host-delegated combinators <code>promise::all</code>/<code>race</code>/<code>all_settled</code>/<code>any</code>/<code>value</code> (all <code>@suspending</code>): the guest kicks off N host operations un-awaited (a Promise-returning call via <code>js::apply</code> returns the pending handle) then joins them with one <code>@await</code>; genuine concurrency, proven (max-in-flight = 2). REMAINING (the still-partial native path): the generic <code>Poll[T] := $Pending | $Ready value T</code> sum (removes the negative-sentinel constraint), guest-side host-async integration (a <code>Future</code> backed by a real Promise woken by the F1b reactor), spawn/microtask-drain. So the data model + concurrent scheduler + host-delegated concurrency all ship; the native generic Poll + guest-side future↔Promise wiring are the refinements.",
...
<header class="hero"><div class="wrap" style="padding-bottom:0">
  ...
  <div class="meta">Progress (2026-06-10): <b style="color:var(--mono)">Phases 0–2 ✓ — the v1.0 FFI gate is CLOSED.</b> C0/M0/F0a;
  F1a object handles; F1b blocking <code>@await</code> productized (dual-backend, JSPI + Asyncify); C1 closures + C2
  (escape gate AND host-callable closures, leak-free under <code>--target=wasm-gc</code>, Bun round-trip verified). <b style="color:#d6a700">Phase 3 ◐</b> —
  F3 poll-reactor CORE + host-delegated concurrency (the <code>promise</code> module) ship; the native generic <code>Poll[T]</code> +
  guest-side future↔Promise wiring remain. <b>Capability model (K1–K8) not started — by design (post-v1.0).</b>
