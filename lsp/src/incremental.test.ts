...
describe('LSP rides the incremental compiler Workspace', () => {
    ...
    test('cross-file: @use resolves with no false unbound-identifier diagnostics', () => {
        ...
        const doc = ws.update(b, `r := helper(41);`)!
    ...
    test('definition + references + hover resolve through the model', () => {
        ...
        // cursor on `add` in the call (line 2 = LSP line 1; 'add' starts at
        // 0-based col 5 in 's := add(1, 2);', so callCol = 5)
        const callLine = 1
        const callCol = 5
    ...
    test('rename rewrites the definition and all references', () => {
        ...
        const u = uri('ren.si')
        ws.update(u, `@fn add x, y := { x + y };\ns := add(1, 2);`)
        const edit = ws.compiler.rename(u, 2, 6, 'plus')   // cursor on `add` in the call
    ...
    test('signature help reports the active parameter inside a call', () => {
        const ws = new Workspace()
        const u = uri('sig.si')
        //  3: s := add(1, 2);  — cursor on the 2nd arg ('2' at col 13)
        ws.update(u, `\\\\ add (Int, Int) -> Int\n@fn add x, y := { x + y };\ns := add(1, 2);`)
        const help = ws.compiler.signatureHelp(u, 3, 13)
