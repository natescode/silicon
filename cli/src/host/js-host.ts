...
function buildImports(state: HostState, write: (s: string) => void) {
    ...
    const imports: WebAssembly.Imports = {
        ...
        path: {
            ...
            format: (pathObject: any) => allocLenString(require('node:path').format(pathObject)),
            is_absolute: (path: number) => require('node:path').isAbsolute(readLenString(path)),
            join: (paths: any) => allocLenString(require('node:path').join(...paths)),
            ...
            parse: (path: number) => require('node:path').parse(readLenString(path)),
            relative: (from: number, to: number) => allocLenString(require('node:path').relative(readLenString(from), readLenString(to))),
            resolve: (paths: any) => allocLenString(require('node:path').resolve(...paths)),
        ...
        os: {
            ...
            loadavg: () => require('node:os').loadavg(),
            machine: () => allocLenString(require('node:os').machine()),
            network_interfaces: () => require('node:os').networkInterfaces(),
            ...
            user_info: (options: any) => require('node:os').userInfo(options),
        ...
        bun: {
            ...
            plugin: (options: any) => Bun.plugin(options),
            ...
            serve: (options: any) => Bun.serve(options),
            ...
            spawn: (options: any) => Bun.spawn(options),
            spawn_sync: (options: any) => Bun.spawnSync(options),
        ...
        response: {
            ...
            json_static: (data: any) => Response.json(data),
            // === /bindgen:module response ===
        },
        request: {
        ...
        fs: {
            ...
            fstat_sync: (fd: number, options: any) => require('node:fs').fstatSync(fd, options),
            ...
            statfs_sync: (path: any, options: any) => require('node:fs').statfsSync(path, options),
