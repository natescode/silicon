# Third-Party Licenses — `sgl` binary

The standalone `sgl` binary is built with `bun build --compile`, which embeds
the **Bun** runtime — and, statically within Bun, **JavaScriptCore/WebKit** and
a number of other libraries. Silicon's own code is MIT-licensed (see the
bundled `LICENSE`). The embedded Bun runtime and its components are covered by
the licenses below, reproduced verbatim from **Bun v1.3.14**'s `LICENSE.md`.

Of note: Bun statically links **JavaScriptCore/WebKit (LGPL-2)**. Per LGPL-2,
the application is distributed in object form so the LGPL'd library can be
modified and relinked; Bun's patched WebKit source and relink instructions are
documented below (and at <https://github.com/oven-sh/webkit>).

The `sgl` binary does **not** bundle `wabt` or `binaryen` — the compiler
assembles WebAssembly with its own emitter.

---

## Bun — https://github.com/oven-sh/bun (v1.3.14) — LICENSE.md (verbatim)
