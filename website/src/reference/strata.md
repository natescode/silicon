...
4. **`CompilerAPI`** — The `Compiler::*` surface strata bodies dispatch into is documented in `docs/compiler-api.md` and implemented in `src/compiler-api/index.ts`.
...
| Type        | Named types the type system understands                               | Hard-coded primitives (`Int`, `Float`, `Bool`, `String`); user-defined types via `@enum` / `@type` (alias) / `@type_distinct` |
